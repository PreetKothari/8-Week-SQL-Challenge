...
SELECT s.customer_id, SUM(m.price) AS total_money_spent FROM dannys_diner.sales s
...
FROM (SELECT 
	   ...
	   FROM menu m
	   JOIN sales s ON m.product_id = s.product_id) AS o
...
SELECT product_name, customer_id FROM ordered_sales
WHERE order_rank = 1
GROUP BY customer_id, product_name;

/* 
SELECT *, ROW_NUMBER() OVER(ORDER BY order_date ASC) as order_rank
FROM sales;
SELECT *, RANK() OVER(ORDER BY order_date ASC) as order_rank
FROM sales;
...
all customers?*/
SELECT m.product_name, COUNT(s.product_id) AS no_of_purchases FROM menu m
JOIN sales s ON m.product_id = s.product_id
...
SELECT customer_id, product_name, no_of_purchases FROM popular_sales
...
SELECT j.customer_id, mn.product_name FROM joined_as_member j
...
iNNER JOIN dannys_diner.sales s ON s.customer_id = mm.customer_id 
							 AND s.order_date < mm.join_date)
SELECT bm.customer_id, mn.product_name FROM before_joined_as_member bm
INNER JOIN dannys_diner.menu mn ON bm.product_id = mn.product_id
								   AND bm.row_num = 1
ORDER BY bm.customer_id; 

/*8. What is the total items and amount spent for each member before they became a member?*/
SELECT mm.customer_id, COUNT(s.product_id) AS total_items,
...
WITH points_cte AS (
SELECT s.customer_id, s.order_date, mn.product_id, mn.product_name,
    CASE
...
SELECT m.customer_id, SUM(p.points) AS total_points
FROM dannys_diner.members m
INNER JOIN points_cte p ON m.customer_id = p.customer_id
